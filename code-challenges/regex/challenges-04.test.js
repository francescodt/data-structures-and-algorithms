...
const noPunctuation = str => {
    let noPuncArr = [];
    let thisRegex = /(\b[A-Za-z0-9]\w*\s)/g;
        return str.match(thisRegex) || [];
};
...
let hangman = (str) => {
    ...
    return str.replace(vowelRegex, '_');
};
...
const findShells = (str) => {
    const findShells = (str) => {
        let newShells = [];
        let shellRegex = /\w*ells\b/ig;
        if (str.match(shellRegex)) {
            newShells = str.match(shellRegex);
          } 
          return newShells;
    };
}
