...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(pokeStat => pokeStat.baseStat > minBaseStat)
};
...
const getStatName = (arr, minBaseStat) => {
  let filteredStats = arr.filter(pokeStat => pokeStat.baseStat > minBaseStat)
  return filteredStats.map(filtered => filtered.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(house => !house.children)
};
...
const evenOddNumericValues = (arr) => {
  let nonNumerics = arr.filter((eachInput) => {
